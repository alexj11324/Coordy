...
# Coordy S0 validation (archived research)

This directory is the frozen Python research harness. The product lives at the
repository root (Electron desktop + Rust daemon). Do not treat this tree as the
runtime, and do not send its Screening pipeline into `coordyd`.
