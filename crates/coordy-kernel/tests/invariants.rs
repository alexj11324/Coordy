...
#[test]
fn built_in_planning_skill_is_scoped_to_private_chat_runs() {
    let h = setup();
    let actor = Actor::Principal {
        id: h.alice.clone(),
    };
    let ordinary_task_id = h
        .kernel
        .submit_sync(cmd(
            actor.clone(),
            Command::CreateTask {
    ...
    h.kernel
        .submit_sync(cmd(
            actor.clone(),
            Command::SetTaskStatus {
                task_id: ordinary_task_id.clone(),
                status: "open".into(),
            },
        ))
        .unwrap();
    let ordinary_run_id = h
        .kernel
        .submit_sync(cmd(
            actor.clone(),
            Command::StartRun {
    ...
    let ordinary_prompt = h
        ...
        .prompt
        .clone();
    assert!(!ordinary_prompt.contains("Built-in Skill: Conversational task planning"));
    ...
    let chat_prompt = &world.run(&chat_run_id).unwrap().prompt;
    assert!(chat_prompt.contains("Built-in Skill: Conversational task planning"));
    assert!(chat_prompt.contains(&h.workspace_id));
    assert!(chat_prompt.contains(&chat_id));
    assert!(chat_prompt.contains(&chat_run_id));
    assert!(chat_prompt.contains(&h.a1));
    assert!(chat_prompt.contains(&h.a2));
    assert!(!chat_prompt.contains("\"name\": \"B1\""));
    let chat_events = world
        .run_events
        .iter()
        .filter(|event| event.run_id == chat_run_id)
        .collect::<Vec<_>>();
    assert!(chat_events
        .iter()
        .any(|event| { event.kind == "message" && event.payload == "user: Split this goal" }));
    assert!(chat_events
        .iter()
        .all(|event| !event.payload.contains("bounded assignee catalog")));
    assert!(
        world.skills.is_empty(),
        "the built-in skill is not workspace data"
    );
}

#[test]
fn chat_run_rejects_missing_mismatched_or_private_context_without_creating_runs() {
    let h = setup();
    ...
    for _ in 0..2 {
        ...
        h.kernel
            ...
            .unwrap();
        chats.push((chat_id, task_id));
    }

    let run_count = h.kernel.export_world().runs.len();
    let missing = h
        .kernel
        .submit_sync(cmd(
            alice.clone(),
            Command::StartRun {
                task_id: chats[0].1.clone(),
                source: RunSource::Acp {
                    prompt: "plan".into(),
                },
                agent_id: Some(h.a1.clone()),
    ...
    let mismatched = h
        ...
        .submit_sync(cmd(
            ...
            Command::StartRun {
                ...
                agent_id: Some(h.a1.clone()),
                chat_id: Some(chats[1].0.clone()),
                trigger: "chat".into(),
            },
        ))
        .unwrap_err();
    assert_eq!(mismatched.code, "invalid");
    ...
    let wrong_trigger = h
        ...
        .submit_sync(cmd(
            ...
            Command::StartRun {
                ...
                agent_id: Some(h.a1.clone()),
                chat_id: Some(chats[0].0.clone()),
                trigger: "issue".into(),
            },
        ))
        .unwrap_err();
    assert_eq!(wrong_trigger.code, "invalid");
    ...
    let private = h
        .kernel
        .submit_sync(cmd(
            Actor::Principal { id: h.bob },
            Command::StartRun {
                task_id: chats[0].1.clone(),
                source: RunSource::Acp {
                    prompt: "steal context".into(),
                },
                agent_id: Some(h.a1),
                chat_id: Some(chats[0].0.clone()),
                trigger: "chat".into(),
        ...
        .unwrap_err();
    assert_eq!(private.code, "denied");
    assert_eq!(h.kernel.export_world().runs.len(), run_count);
}

#[test]
fn assistant_plan_artifact_creates_reviewable_revision_without_applying_tasks() {
    let h = setup();
    let actor = Actor::Principal {
        id: h.alice.clone(),
    };
        ...
        .unwrap()
    {
        View::Chat {
            ...
            let proposal = task_plan.expect("reviewable proposal");
            assert_eq!(proposal.revision, 1);
            assert_eq!(proposal.draft, draft);
    ...
    match h
        ...
        .unwrap()
    {
        View::Run { events, .. } => assert!(events.iter().any(|event| {
            event.kind == "message"
                && event.payload == "assistant: 我把工作分成两个阶段，请先检查依赖和负责人。"
        })),
        _ => panic!("run"),
    }
    let world = h.kernel.export_world();
...
fn unsupported_plan_artifact_stays_visible_and_inert() {
    ...
    let parent_id = h
        ...
        .unwrap()
        .to_string();
    let mut draft = plan_draft(&h, TaskPlanParent::Existing { task_id: parent_id });
    draft.version = "COORDY_TASK_PLAN_V2".into();
    let content = format!(
        "unsupported\n```COORDY_TASK_PLAN_V1\n{}\n```",
        serde_json::to_string(&draft).unwrap()
    );
        ...
        .unwrap()
    {
        View::Chat {
            ...
            assert!(task_plan.is_none());
            assert!(task_plan_error
                .unwrap()
                .contains("unsupported task plan version"));
        }
        _ => panic!("chat"),
    }
    let world = h.kernel.export_world();
    assert!(world.task_plan_proposals.is_empty());
    assert!(world.task_plan_applications.is_empty());
    assert!(world.run_events.iter().any(|event| {
        event.run_id == draft.source_run_id && event.payload == format!("assistant: {content}")
    }));
}

#[test]
fn incomplete_plan_artifact_errors_only_after_chat_run_finishes() {
    let h = setup();
    let parent_id = h
        .kernel
        .submit_sync(cmd(
...
fn invalidate_producer(h: &Harness, producer: &str) {
    bind_demo_repo(h);
    create_worktree(h, producer);
    h.kernel
        .submit_sync(cmd(
...
fn graph_review_runs(h: &Harness) -> Vec<coordy_protocol::RunView> {
    alice_runs(h)
        .into_iter()
        .filter(|run| run.trigger == "graph_review")
        .collect()
}

fn eval_ready(h: &Harness) -> coordy_protocol::GraphEvaluationView {
    match h
        .kernel
        .view_sync(q(
            alice_actor(h),
        ...
        .unwrap()
    {
        View::GraphEvaluation(eval) => eval,
        other => panic!("expected graph evaluation, got {other:?}"),
    }
}

#[test]
fn validation_decision_rejects_expired_generation() {
    let h = setup();
    let producer = issue_title(&h, "api");
    let consumer = issue_title(&h, "ui");
    let dep_id = declare_dep(&h, &consumer, &producer, "repo");
    ...
    let err = h
        ...
        .submit_sync(cmd(
            ...
            Command::ValidationDecision {
                ...
                expected_generation: generation.saturating_sub(1),
                decision: ValidationChoice::Reaffirm,
                evidence_refs: vec!["packet".into()],
                rationale: "rubber stamp".into(),
                validator_run_id: None,
            },
        ))
        .unwrap_err();
...
fn reaffirm_rejects_stale_generation_and_does_not_start_run() {
    validation_decision_rejects_expired_generation();
}

#[test]
fn validation_hold_keeps_downstream_out_of_ready_set() {
    let h = setup();
    let producer = issue_title(&h, "api");
    let consumer = issue_title(&h, "ui");
    let dep_id = declare_dep(&h, &consumer, &producer, "repo");
    ...
    h.kernel
        .submit_sync(cmd(
            ...
            Command::ValidationDecision {
                dependency_id: dep_id,
                expected_generation: generation,
                decision: ValidationChoice::Hold,
                evidence_refs: vec!["review-packet".into()],
                rationale: "hold until next plan".into(),
                validator_run_id: None,
            },
        ))
        .unwrap();
    let edge = alice_deps(&h)
        .into_iter()
        .find(|dep| dep.target.id == consumer)
        .expect("edge");
    assert_eq!(edge.state, GraphEdgeState::PendingValidation);
    let eval = eval_ready(&h);
    assert!(!eval.ready_nodes.contains(&consumer));
}

#[test]
fn human_validation_uses_the_same_command_as_conductor() {
    let h = setup();
    ...
    h.kernel
        .submit_sync(cmd(
            ...
            Command::ValidationDecision {
                ...
                expected_generation: generation,
                decision: ValidationChoice::Reaffirm,
                evidence_refs: vec!["human".into()],
                rationale: "inspector".into(),
                validator_run_id: None,
            },
        ))
        .unwrap();
...
#[test]
fn executor_reaffirm_prefix_does_not_clear_stale_edge() {
    let h = setup();
    let producer = issue_title(&h, "api");
    let consumer = issue_title(&h, "ui");
    let dep_id = declare_dep(&h, &consumer, &producer, "repo");
    ...
    h.kernel
        .submit_sync(cmd(
            ...
            Command::IngestHarnessEvent {
                ...
                event: HarnessEvent::Message {
                    role: "assistant".into(),
                    content: format!("REAFFIRM: {dep_id} generation={generation}"),
                },
            },
        ))
        .unwrap();
    ...
    assert_eq!(producer_run.role, RunRole::Executor);
}

#[test]
fn conductor_review_prefix_without_generation_is_ignored() {
    let h = setup();
    set_conductor(&h, &h.a1);
    let producer = issue_title(&h, "api");
    let consumer = issue_title(&h, "ui");
    ...
    let review = graph_review_runs(&h)
        ...
        .find(|run| run.task_id == consumer)
        .expect("review run");
    assert_eq!(review.role, RunRole::ConductorReview);
    h.kernel
        .submit_sync(cmd(
            Actor::Daemon,
            Command::IngestHarnessEvent {
                run_id: review.id.clone(),
                event: HarnessEvent::Message {
                    role: "assistant".into(),
    ...
    h.kernel
        .submit_sync(cmd(
            ...
            Command::IngestHarnessEvent {
                ...
                event: HarnessEvent::Message {
                    role: "assistant".into(),
                    content: format!("HOLD: {dep_id} generation={generation}"),
                },
            },
        ))
        .unwrap();
...
#[test]
fn only_assistant_review_messages_can_apply_validation_decisions() {
    let h = setup();
    set_conductor(&h, &h.a1);
    let producer = issue_title(&h, "api");
    let consumer = issue_title(&h, "ui");
    ...
    assign_a1(&h, &consumer);
    invalidate_producer(&h, &producer);
    let review = graph_review_runs(&h).pop().expect("review run");
    let generation = alice_deps(&h)
        .into_iter()
        .find(|dep| dep.id == dep_id)
        .expect("dependency")
        .generation;
    let decision = format!("REAFFIRM: {dep_id} generation={generation}");
    ...
    ingest_assistant(&h, &review.id, &decision);
    assert_eq!(
        alice_deps(&h)
            .into_iter()
            .find(|dep| dep.id == dep_id)
...
#[test]
fn inconclusive_successful_review_pauses_without_immediate_restart() {
    for outcome in ["silent", "missing_generation", "wrong_generation"] {
        let h = setup();
        set_conductor(&h, &h.a1);
        let producer = issue_title(&h, "api");
        let consumer = issue_title(&h, "ui");
        ...
        match outcome {
            ...
            "wrong_generation" => ingest_assistant(
                &h,
                &review.id,
                &format!("REAFFIRM: {dep_id} generation={}", generation + 1),
            ),
            _ => {}
        }

        ingest_session_ok(&h, &review.id);
        assert_eq!(graph_review_runs(&h).len(), 1, "{outcome} restarted inline");
        let world = h.kernel.export_world();
        assert!(world.node_attempts.iter().any(|attempt| {
            attempt.run_id.as_deref() == Some(review.id.as_str())
                && attempt.lease_status == "paused"
        }));
        assert_eq!(
            alice_deps(&h)
                .into_iter()
        ...
        h.kernel
            ...
            .unwrap();
        assert_eq!(
            graph_review_runs(&h).len(),
            2,
            "{outcome} was not retryable"
        );
    }
}

#[test]
fn run_backed_validation_decision_is_consumed_once() {
    let h = setup();
    set_conductor(&h, &h.a1);
    let producer = issue_title(&h, "api");
    let consumer = issue_title(&h, "ui");
    ...
    ingest_assistant(
        ...
        &review.id,
        &format!(
            "HOLD: {dep_id} generation={generation}\nREMOVE: {dep_id} generation={generation}"
        ),
    );
    ingest_assistant(
        &h,
        &review.id,
        &format!("REMOVE: {dep_id} generation={generation}"),
    );
    assert_eq!(
        alice_deps(&h)
            .into_iter()
            .find(|dep| dep.id == dep_id)
            .expect("first decision must remain authoritative")
            .state,
        GraphEdgeState::PendingValidation
    );
    assert!(h.kernel.export_world().node_attempts.iter().any(|attempt| {
        attempt.run_id.as_deref() == Some(review.id.as_str()) && attempt.lease_status == "succeeded"
    }));
}

#[test]
fn terminal_review_attempts_retry_as_conductor_reviews() {
    for terminal in ["failed", "cancelled"] {
        ...
        let generation = alice_deps(&h)
            ...
            .expect("dependency")
            .generation;
        let stale_decision = h.kernel.submit_sync(cmd(
            daemon(),
            Command::ValidationDecision {
                dependency_id: dep_id.clone(),
                expected_generation: generation,
                decision: ValidationChoice::Reaffirm,
                evidence_refs: vec!["terminal-review".into()],
                rationale: "terminal review must not decide".into(),
                validator_run_id: Some(review.id.clone()),
            },
        ));
        assert_eq!(stale_decision.unwrap_err().code, "denied");
        ...
        let retry_run = alice_runs(&h)
            .into_iter()
            .find(|run| run.id == retry_id)
            .expect("retry run");
        assert_eq!(retry_run.role, RunRole::ConductorReview);
        assert_eq!(retry_run.trigger, "graph_review");
        assert_eq!(retry_run.agent_id, h.a1);
        ingest_assistant(
            &h,
            &retry_run.id,
            &format!("HOLD: {dep_id} generation={generation}"),
        );
        assert_eq!(
            alice_deps(&h)
                .into_iter()
...
#[test]
fn conductor_change_fences_terminal_review_and_dispatches_current_conductor() {
    let h = setup();
    set_conductor(&h, &h.a1);
    let producer = issue_title(&h, "api");
    let consumer = issue_title(&h, "ui");
    ...
    assign_a1(&h, &consumer);
    invalidate_producer(&h, &producer);
    let old_review = graph_review_runs(&h).pop().expect("old review");
    h.kernel
        .submit_sync(cmd(
            daemon(),
            Command::IngestHarnessEvent {
                run_id: old_review.id.clone(),
                event: HarnessEvent::Tool {
                    name: coordy_protocol::HARNESS_SESSION_TOOL.into(),
                    input: String::new(),
                    output: "failed".into(),
    ...
    let current_review = graph_review_runs(&h)
        .into_iter()
        .find(|run| run.status == "running")
        .expect("current conductor review");
    assert_eq!(current_review.agent_id, h.a2);
    let world = h.kernel.export_world();
    assert!(world.node_attempts.iter().any(|attempt| {
        attempt.run_id.as_deref() == Some(old_review.id.as_str())
            && attempt.lease_status == "paused"
    }));
    assert!(world.node_attempts.iter().any(|attempt| {
        attempt.run_id.as_deref() == Some(current_review.id.as_str())
            && attempt.lease_status == "running"
    }));
    assert!(h
        .kernel
        .submit_sync(cmd(
...
#[test]
fn cleared_conductor_retry_does_not_leave_an_orphan_claim() {
    let h = setup();
    set_conductor(&h, &h.a1);
    let producer = issue_title(&h, "api");
    let consumer = issue_title(&h, "ui");
    ...
    h.kernel
        ...
        .unwrap();
    let before = h.kernel.export_world().node_attempts.len();
    assert!(h
        .kernel
        .submit_sync(cmd(
            alice_actor(&h),
            Command::RetryRun { run_id: review.id },
        ))
        .is_err());
    assert_eq!(h.kernel.export_world().node_attempts.len(), before);

    set_conductor(&h, &h.a2);
    assert!(graph_review_runs(&h)
        .iter()
        .any(|run| run.status == "running" && run.agent_id == h.a2));
}

#[test]
fn archived_agent_cannot_be_restored_as_workspace_conductor() {
    let h = setup();
    set_conductor(&h, &h.a1);
    h.kernel
        .submit_sync(cmd(
    ...
    assert_eq!(
        h.kernel
            .export_world()
            .workspace(&h.workspace_id)
            .and_then(|workspace| workspace.conductor_agent_id.as_deref()),
        Some(h.a1.as_str())
    );
}
...
fn conductor_reviews_stale_edge_then_reaffirm_starts_executor() {
    ...
    ingest_assistant(
        &h,
        &conductor_run,
        &format!("REAFFIRM: {dep_id} generation={generation}"),
    );
    assert!(alice_deps(&h)
        .iter()
        .any(|dep| dep.id == dep_id && dep.valid));
...
fn executor_agent_cannot_reaffirm_but_principal_and_conductor_can() {
    ...
    let conductor_denied = h
        ...
        .unwrap_err();
    assert_eq!(conductor_denied.code, "denied");
    h.kernel
        .submit_sync(cmd(
            alice_actor(&h),
            Command::ValidationDecision {
                ...
                decision: ValidationChoice::Reaffirm,
                evidence_refs: vec!["human-review".into()],
                rationale: "principal reviewed the current generation".into(),
                validator_run_id: None,
            },
        ))
        .unwrap();
...
fn declared_graph_retries_new_generation_after_completed_run_and_capacity_release() {
    ...
    let fingerprints: std::collections::HashSet<_> = h
        ...
        .node_attempts
        .into_iter()
        .filter(|attempt| attempt.node_id == consumer && attempt.role == RunRole::Executor)
...
fn completed_review_reconciles_waiting_reviews_and_reaffirmed_execution() {
    ...
    let generation = alice_deps(&h)
        .into_iter()
        .find(|dep| dep.id == dep_a)
        .expect("first dependency")
        .generation;
    ingest_assistant(
        &h,
        &first_review.id,
        &format!("HOLD: {dep_a} generation={generation}"),
    );
    ...
    let generation = alice_deps(&same_agent)
        .into_iter()
        .find(|dep| dep.id == dep_id)
        .expect("dependency")
        .generation;
