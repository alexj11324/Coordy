...
`get_context.py` shows the active task's `status` field. Route by `status` + artifact presence. This command replaces the user needing to remember the Trellis flow; implementation authority comes from the user's explicit implementation request once the planning evidence gate passes.
...
- `status=planning` + required artifacts complete + required jsonl curated or inline mode → **1.4** (if the request explicitly asks for implementation and no unresolved user-owned decision remains, run `task.py start`; otherwise stop after planning or ask the genuine decision)
