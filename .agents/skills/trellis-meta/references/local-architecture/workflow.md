...
| Change the wrap-up flow | Update Phase 3 and `finish-work` related descriptions. Note the current split: Phase 3.4 = autonomous AI-driven commits of verified task-owned changes excluding `.trellis/tasks/` and `.trellis/workspace/`, asking only when overlapping ownership cannot be safely separated; Phase 3.5 = `/finish-work` (archive + record session), whose scripts auto-commit those bookkeeping paths. `/finish-work` refuses to run if current-task changes remain dirty. |
