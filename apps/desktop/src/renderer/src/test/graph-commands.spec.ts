...
describe("graph inspector commands", () => {
  ...
  it("does not reaffirm assigned, blocker, valid, or generation-less edges", () => {
    ...
    expect(
      reaffirmCommandForStaleEdge({
        edgeId: "blocker:task_api:task_ui",
        stale: true,
        generation: 2,
      }),
    ).toBeNull();
    expect(
      reaffirmCommandForStaleEdge({
        edgeId: "assigned:agent:task",
        stale: true,
        generation: 2,
      }),
    ).toBeNull();
    expect(
      reaffirmCommandForStaleEdge({ edgeId: "dep:dep_1", stale: true }),
    ).toBeNull();
  });
  ...
  it("removes only generation-bound kernel dependency edges through validation", () => {
    ...
    expect(
      removeCommandForDependencyEdge("blocker:task_api:task_ui"),
    ).toBeNull();
  ...
  it("declares a dependency from upstream source to the selected target", () => {
    expect(
      declareDependencyCommand("ws_1", "task_api", "task_ui", "  "),
    ).toEqual({
