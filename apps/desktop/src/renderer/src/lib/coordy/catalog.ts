...
export const SKILL_STARTERS: SkillStarter[] = [
  {
    ...
    body: [
      ...
      "边的方向是上游 source 到下游 target。改上游产物后，只失效 Consumes 且 source 等于变更节点的边。失效后须带当前 generation 确认（或移除后重规划）才能再跑。",
      "失效边由总管批准，不要让执行者自己确认。总管在内部 trigger 为 graph_review 的回合用助手消息写：",
