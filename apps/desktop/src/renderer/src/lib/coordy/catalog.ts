...
export const SKILL_STARTERS: SkillStarter[] = [
  {
    ...
    body: [
      ...
      "ACCEPTANCE: <怎样算完成>",
      "",
      "DEPENDS 第一段必须是内核能唯一解析的事项 id、智能体 id、契约 id 或编号（例如 task_… / COOR-12）。自由描述不会建边。第二段是实体，默认 repo。",
      "",
      "工作区指定图总管后，绿灯且已指派的事项由内核按 DAG 自动开工；未指定总管时仍由成员点开工。",
      "改 API、契约或仓库实体后，只打脏连着该节点的下游。失效边由总管批准，不要让执行者自己确认。",
      "总管在 trigger 为 graph 的回合用助手消息写：",
      "REAFFIRM: <dependency_id>",
      "HOLD: <dependency_id>",
      "批准后内核会用重规划提示开工执行者。HOLD 或不写则继续挡住。成员仍可在 Inspector 覆盖确认。",
      "",
