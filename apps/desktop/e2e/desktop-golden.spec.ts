...
type BootstrapIds = Pick<
  GoldenIds,
  "workspaceId" | "principalId" | "daemonPid"
>;
type GoldenRuntime = {
...
async function waitForDevtoolsPort(
  ...
  throw new Error(
    `Electron DevTools port did not appear within 10s: ${logs()}`,
  );
}
...
async function cleanupGoldenRuntime(runtime: GoldenRuntime): Promise<{
  ...
  if (
    runtime.page &&
    runtime.application.exitCode === null &&
    runtime.application.signalCode === null
  ) {
    await runtime.page
      .evaluate(() => window.coordy.quit())
      .catch(() => undefined);
  }
  const exited = await waitForExit(runtime.application, 10_000);
  const gracefulExit =
    exited &&
    runtime.application.exitCode === 0 &&
    runtime.application.signalCode === null;
  if (
    !exited &&
    runtime.application.exitCode === null &&
  ...
  const daemonStopped =
    runtime.daemonPid <= 0 || !processExists(runtime.daemonPid);
  if (
    !runtime.runtimeRoot.startsWith(join(runtime.temporaryRoot, "cdy-e2e-"))
  ) {
    throw new Error(
      `refusing to remove unexpected golden path: ${runtime.runtimeRoot}`,
    );
  }
...
test("clean Electron state completes the real coordyd product spine and terminates its child", async () => {
  ...
  expect(
    existsSync(main),
    "build the desktop before running the golden flow",
  ).toBe(true);
  expect(
    existsSync(daemon),
    "build coordyd before running the golden flow",
  ).toBe(true);
  expect(
    existsSync(cli),
    "build coordy so the deterministic ACP stub is discoverable",
  ).toBe(true);

  const logs: string[] = [];
  const application = spawn(
    electronPath,
    [main, `--user-data-dir=${userData}`, "--remote-debugging-port=0"],
    {
      env: {
        ...process.env,
  ...
  try {
    const port = await waitForDevtoolsPort(application, userData, () =>
      logs.join(""),
    );
    ...
    await expect
      .poll(() =>
        page.evaluate(async () => {
          ...
          const workspaces = await bridge.view({
            actor: { type: "daemon" },
            query: { type: "Workspaces" },
          });
          if (workspaces.type !== "Workspaces" || !workspaces.items[0])
            return false;
          ...
          return (
            principals.type === "Principals" && principals.items.length > 0
          );
        }),
      )
      .toBe(true);

    const bootstrapIds = await page.evaluate(
      async (): Promise<BootstrapIds> => {
        const bridge = window.coordy;
        const health = await bridge.view({
          actor: { type: "daemon" },
          query: { type: "Health" },
        });
        const workspaces = await bridge.view({
        ...
        if (
          !runtimes.some(
            (runtime) => runtime.id === "coordy-stub" && runtime.installed,
          )
        ) {
          throw new Error(
            "deterministic coordy-stub runtime was not discovered",
          );
        }
        return {
          workspaceId,
    ...
    await expect(page.locator("body")).toContainText("已结束。", {
      timeout: 15_000,
    });
    const openIssue = page.getByRole("button", {
      name: "打开事项",
      exact: true,
    });
    ...
    const ids = await page.evaluate(
      async ({ workspaceId, principalId }): Promise<GoldenIds> => {
        const bridge = window.coordy;
        const health = await bridge.view({
          actor: { type: "daemon" },
          query: { type: "Health" },
        ...
        const task = board.tasks.find(
          (item) => item.title === "Golden Electron issue",
        );
        const run = task
          ? runs.items.find((item) => item.task_id === task.id)
          : undefined;
        if (!agent || !task || !run)
          throw new Error("UI-created golden entities are missing");
        return {
          workspaceId,
          principalId,
          agentId: agent.id,
    ...
    const runProjection = await page.evaluate(
      async (runId) =>
        window.coordy.view({
          actor: { type: "daemon" },
          query: { type: "Run", run_id: runId },
    ...
    expect(runProjection.events.some((event) => event.kind === "tool")).toBe(
      true,
    );
  } finally {
    ...
    activeRuntime = null;
    expect(
      cleanup.daemonStopped,
      "coordyd child remained alive after quit",
    ).toBe(true);
    expect(cleanup.tempRemoved, "isolated golden state was not removed").toBe(
      true,
    );
    expect(
      cleanup.gracefulExit,
      `Electron did not quit cleanly (exit=${runtime.application.exitCode}, signal=${runtime.application.signalCode}): ${logs.join("")}`,
    ).toBe(true);
  }
});
