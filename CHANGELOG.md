...
- Add an explicit fail-closed legacy causal-checkpoint reconciliation path: unverifiable old rows become provenance-bound `UNASSESSABLE_OUTPUT_FAILURE` records with a preserved snapshot, while only durable accepted results are recovered into the 387-item human review queue.
- Require accepted causal results to carry completed API request/response provenance and recover a bound unassessable placeholder if a process exits between dispatch binding and checkpoint replacement.
- Close the pragmatic Temporal S0 screening with `STOP`: the final two primary-suspect cases carrying deterministic outcome signals were a literal `0 failed` false positive and ordinary UI-test iteration that later passed, leaving no confirmed compaction-caused engineering loss; keep Cross-Agent/Type B separate for its own subsequent audit.
- Retract that premature Temporal `STOP`: targeted disagreement review and two consequence inspections did not replace T0-T5 causal review of every final primary `SUSPECT`; continue through confirmed-case detector and baseline comparison before making a Temporal Screening decision.
- Close the independent Cross-Agent S0 screening with `STOP`: 197 structural patch collisions reduce to 20 session pairs across 4 Goal roots, while a consequence-ranked audit found existing ownership, coordination, verification, and stop-on-conflict behavior rather than unaware stale-plan execution with engineering loss; record the combined startup-screening result as `STOP / STOP`.
...
- Run the explicitly authorized Luna-only causal prelabel pass over all 387 source-bound cases (483 episodes), preserve two transport/semantic failures as unassessable, and emit a 166-item human triage queue while keeping machine labels out of Ground Truth.
- Add triaged human adjudication support: clear ordinary/irrelevant machine items remain auxiliary, while only human-confirmed positive, near-miss, and difficult-negative items can enter the bound Ground Truth artifact.
- Enforce exact FULL/TRIAGED Ground Truth scope pairing and bind the human review manifest context hash to the current causal-input artifact before adjudication.
